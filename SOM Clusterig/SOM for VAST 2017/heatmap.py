...
class HeatMap(object):
    def __init__(self, names, names2 = [],  filename = 'som.csv', labourname = 'key points.csv', mapname = 'map.jpg'):
        self.names = names
        self.names2 = names2
        self.img = mpimg.imread(mapname)
        self.file = pd.read_csv(filename)
        self.points = pd.read_csv(labourname).ix[1:41,2:4].values
    ...
    def process(self):
        ...
        plt.scatter(self.points[:,0], self.points[:,1], s= 100*sum1.astype('int64'), color= colors)
    ...
    def groupProcess(self):
        ...
        axes[0].scatter(self.points[:,0], self.points[:,1], s= 100*sum1.astype('int64'), color= colors)
        ...
        axes[1].scatter(self.points[:,0], self.points[:,1], s= 100*sum1.astype('int64'), color= colors)
